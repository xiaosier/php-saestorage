...
define("US_AUTHURL", "https://auth.sinas3.com");
/**
...
class CF_Container
{
     ...
     * @param obj $container name or instance of Container in which the object resides (optional)
     * @package lazypeople<hfutming@gmail.com>
     */
    function delete_object_by_path($path)
    {
        $path_name = NULL;
        if (is_string($path)) {
            $path_name = $path;
        ...
        if (!$path_name) {
            throw new SyntaxException("Path name not set.");
        }
        $file_lists = $this->list_objects(0,NULL,NULL,$path);
        if(count($file_lists) == 0) {
            return true;
        } else {
            foreach ($file_lists as $small) {
                $this->delete_object($small);
            }
            return true;
        }
    }
     ...
     *
     * Make a dir 
     * package by lazypeople<hfutming@gmail.com>
     */
...
class CF_Object
{
    ...
    /**
     * Set content type of Object
     *
     * Set content type of An Object if the content type of Object is unknow
     *
     * @return boolean
     */
    function __set_content_type($type)
    {
     ...
     * @return string
     */
    public function __getMimeType($filename) 
    {
        static $exts = array(
        ...
        $ext = isset($exts[$ext]) ? $exts[$ext] : 'application/octet-stream';
        $this->__set_content_type($ext);
    }
